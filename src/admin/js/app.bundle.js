...
function checkFormValidation(form, array) {
    var validation = true;
    $.each(array, (index, item) => {
        if(item == '') {
            $(`${form} input[name='${index}']`).addClass('is-invalid')
            validation = false;
        }
    });
    return validation;
...
function checkRequiredFormValidation(form, array) {
    var validation = true;
    $.each(array, (index, item) => {
        if($(`${form} input[name='${index}']`).prop('required'))
            if(item == '') {
                $(`${form} input[name='${index}']`).addClass('is-invalid')
                validation = false;
            }
...
function checkRequiredValidation(form, array) {
    var validation = true;
    $.each(array, (index, item) => {
        if($(`${form} input[name='${item.name}']`).prop('required'))
            if(item.value == '') {
                $(`${form} input[name='${item.name}']`).addClass('is-invalid')
                validation = false;
            } else {
                $(`${form} input[name='${item.name}']`).removeClass('is-invalid')
            }
    });
    return validation;
}

jQuery.fn.showSp = function() {
	var o = $(this[0]) // This is the element
	o.addClass('show loader');
	return this; // This is needed so other functions can keep chaining off of this
};

jQuery.fn.hideSp = function() {
	var o = $(this[0]) // This is the element
	o.removeClass('show loader');
	return this; // This is needed so other functions can keep chaining off of this
};